...
public class ConsoleKeys {
    ...
    private static final Logger logger = Logger.getLogger(ConsoleKeys.class.getName());
    ...
    protected void loadKeys(String appName, URL inputrcUrl) {
        ...
        try {
            ...
            try {
                ...
                logger.fine("Loaded user configuration: " + inputrcUrl);
            }
            finally {
                try {
                    input.close();
            ...
            if (inputrcUrl.getProtocol().equals("file")) {
                ...
                if (file.exists()) {
                  logger.log(Level.FINE, "Unable to read user configuration: " + inputrcUrl, e);
                }
            } else {
                logger.log(Level.FINE, "Unable to read user configuration: " + inputrcUrl, e);
            }
                ...
                if (keySeq.equalsIgnoreCase("set")) {
                    ...
                    if (val.length() > 0 && (val.charAt(0) == '\'' || val.charAt(0) == '\"')) {
                        ...
                        try {
                          ...
                          logger.fine("Unable to bind key for unsupported operation: " + val);
                        }
                    }
                }
            } catch (IllegalArgumentException e) {
                logger.log(Level.FINE, "Unable to parse user configuration: ", e);
            }
        }
    }
